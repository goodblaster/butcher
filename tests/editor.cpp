...
static void check_gold(void)
{
	...
	for (size_t c = 0; c < sizeof(cases) / sizeof(cases[0]); c++) {
		...
		if (!hero_set_gold(&h, cases[c].amount, err)) {
	...
	/* Over capacity on an empty inventory. */
	{
		PkPlayerStruct h;
		base_hero(&h, PC_WARRIOR);
		ok(!hero_set_gold(&h, 200001, err), "200001 gold is refused (40 cells x 5000)");
		printf("        %s\n", err);
	...
	/* Existing items must survive; existing gold must be replaced, not added to. */
	{
		...
		ok(hero_set_gold(&h, 3000, err), "re-setting gold with items present");
	...
	/* Capacity shrinks as items take cells. */
	{
		...
		ok(hero_gold_capacity(&h) == 150000, "capacity reflects free cells");
		ok(!hero_set_gold(&h, 150001, err), "over the reduced capacity is refused");
		ok(hero_set_gold(&h, 150000, err), "exactly the capacity is accepted");
	...
	/* Setting gold to zero clears every stack. */
	{
		PkPlayerStruct h;
		base_hero(&h, PC_WARRIOR);
		hero_set_gold(&h, 50000, err);
		ok(hero_set_gold(&h, 0, err), "gold back to zero");
	...
	/* Deterministic: same input, same bytes. */
	{
		...
		hero_set_gold(&a, 33333, err);
		hero_set_gold(&b, 33333, err);
	...
	/* Negative is refused. */
	{
		PkPlayerStruct h;
		base_hero(&h, PC_WARRIOR);
		ok(!hero_set_gold(&h, -1, err), "negative gold refused");
	}
}
